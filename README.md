...
12. [Version 8. MCTS 최종 실험: RAVE 하이퍼파라미터 튜닝](#version-8-mcts-최종-실험-rave-하이퍼파라미터-튜닝)
...
## Version 8. MCTS 최종 실험: RAVE 하이퍼파라미터 튜닝
...
#### 가. 결과 분석: RAVE의 신뢰도가 높을수록 오히려 약해진다
...
- IDDFS-αβ: 정교한 평가 함수와 깊이 우선 탐색을 결합한 강력하고 안정적인 AI.

- RAVE MCTS: 통계적 추론과 샘플링 기반 탐색을 사용하는 유연한 AI.

최종적으로 이 프로젝트의 오셀로 게임에서는 다양한 개선 노력에도 불구하고 RAVE MCTS가 IDDFS-αβ의 성능을 뛰어넘지는 못했다. 이는 오셀로와 같은 게임에서는 잘 만들어진 휴리스틱 기반의 알파-베타 탐색이 순수 통계 기반의 MCTS보다 여전히 강력할 수 있음을 보여준다.

이 프로젝트를 통해 각 알고리즘의 장단점과 한계를 이해하게 되었다. 나아가 비록 간단한 실험일지라도, 실제로 실험을 계획하고 수행하면서 데이터를 통해 의사결정 하는 과정을 경험한 것이 큰 수확이었다.
